...
def sortList(sortList):
    ...
    for i in range(lenList):
        ...
        print('Step 1 =',listToSort)
        if sortList[-1][1] == '1':
            sortList = flipListPart(sortList,lenList - 1,listFlipTemp)
        print('Step 2 =',listToSort)
        sortList = flipListPart(sortList,lenList - int(toSort),listFlipTemp)
        print('Step 3 =',listToSort)
    return(sortList)

listToSort = input('Crepe Pile - ')
if listToSort == 'r':
    listToSort = genRandom(listToSort,listParts)
else:
    listToSort = listToSort.split('')
    
    for i in listToSort:
        idx = listToSort.index(i)
        listToSort[idx] = i.split(',')
print('Start Pile =',listToSort)
